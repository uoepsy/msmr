...
In the example where we model school children's positive mode as a function of how many days of outdoor physical education (PE) they did, when we fit a simple regression model of `lm(mood ~ outdoordays)`, the estimated parameters are two values that define a line - an intercept and a slope (as in @fig-schoolplot1).  
...
The intercept and slope here are 'fixed' in the sense that it does not matter what school a child is from, if they had 0 days of outdoor PE, then our model predicts that when asked to report their positive mood they will score `r round(coef(srmod)[1],1)` (the intercept).  
...
knitr::include_graphics("images_lmm/un_lmm/Slide2.PNG")
```
...
knitr::include_graphics("images_lmm/lmercode.png")
```
...
#| echo: false
#| out-width: "100%"
knitr::include_graphics("images_lmm/lmeroutput.png")
```
...
knitr::include_graphics("images_lmm/mle.png")
```
