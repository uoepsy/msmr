...
\text{Level 1 (Toy):}& \\
\text{R_AGE}_{ij} &= b_{0i} + b_1 \cdot \text{hrs_week}_{ij} + \epsilon_{ij} \qquad \\
\text{Level 2 (Type):}& \\
...
Match the bits from each of the above sections  
...
The question  
&npsb;&npsb;&npsb; "*How do different types of audio interfere with executive functioning*"   means we are interested in the effects of audio type (`audio`) on executive functioning (`SDMT` scores), so we will want:
...
&npsb;&npsb;&npsb; "*... and does this interference differ depending upon whether or not noise-cancelling headphones are used?*"  
