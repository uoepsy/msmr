...
library(patchwork)
neststress <- read_csv("../../data/stressweek_nested.csv")
...
```{r}
#| include: false
dfcr <- read_csv("../../data/stressweek_crossed.csv") |>
  mutate(day=day-1,
...
```{r}
psychstress <- read_csv("../../data/stressweek1.csv")
head(psychstress)
```
...
And we can see in our summary that there is a lot of by-department variation - departments vary in their baseline stress levels with a standard deviation of `r rr[3,5]`, and within departments, participants vary in baseline stress scores with a standard deviation of `r rr[1,5]`. 
...
```{r}
stresscross <- read_csv("../../data/stressweek_crossed.csv")
head(stresscross)
```
