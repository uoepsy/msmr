...
If we assume these pupils are all independent, as our old friend `lm()` would, then our estimate of the residual variability will be smaller. This means that on average our standard errors will be too narrow, any confidence intervals too narrow, our F statistics will be too large, and our p-values too small.  
...
TODO tidy
$$
SE(b_p) = \sqrt{  \frac{MSE}{(1-R^2_{x_p | x_p}) \cdot Var(x_p)} }
$$
:::
...
tribble(
  ~schoolid,~grade,
  "North West Community Campus","50",
  "North West Community Campus","65",
  "North West Community Campus","40",
  "...","...",
  "Paisley Grammar School","75",
  "Paisley Grammar School","80",
  "...","...",
) |> as.data.frame() -> dd
...
row.names(dd)<-c("North West Community Campus","..","Paisley Grammar School","...")
dd
...
1. Estimate the effect of motivation on grades after accounting for school-specific differences in grades.: 
```{r}
#| eval: false
lm(grade ~ motiv + schoolid, data = schoolmot)
```

2. Estimate school-specific differences in grades, and school-specific differences in the effect of motivation on grades. Get _loads_ of coefficients that aren't really of interest. Slightly more difficult to get out an average slope of `grade ~ motiv`, and each school contributes equally to this average (regardless of how many children we have from that school).   
```{r}
#| eval: false
lm(grade ~ motiv * schoolid, data = schoolmot)
```
...
set.seed(123)
bind_rows(
  schoolmot |> filter(schoolid %in% sort(unique(schoolmot$schoolid))[c(1:4)]),
  tibble(
