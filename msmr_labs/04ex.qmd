...
`r qend()`
`r solbegin(show=FALSE, toggle=params$TOGGLE)`
...
orthogonal polynomials for me.
i feel like either would be fine here. 
nothing explicit from the study background about stuff at baseline. There's the stuff about the plateau at 7 or 8, but we can get that from the model plots. orthogonal will allow us to compare the trajectories overall (their linear trend, the 'curviness' and 'wiggliness').  

I've gone for poly3 here because the plot above shows a bit of an S-shape for the bilinguals. 
...
library(merTools)
preddat <- wiggle(preddat, varlist = "isBilingual", list(c("0","1")))
#or:
# preddat <- bind_rows(
#   preddat,
#   preddat |> mutate(isBilingual="1")
# )

bind_cols(preddat, 
          predictInterval(merMod = mod7, newdata = preddat,
...
# pp1 <- preddat
# bts = bootMer(mod7,FUN=function(x) predict(x,newdata=pp1,re.form=NA),nsim=10)
# pp1$pred = predict(mod7, newdata=pp1,re.form=NA)
