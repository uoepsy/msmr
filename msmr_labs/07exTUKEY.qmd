...
```{r}
jobperf <- read_csv("../../data/jobperfbonus.csv")
head(jobperf)
```
...
The data can be found at [https://uoepsy.github.io/data/conduct_probs.csv](https://uoepsy.github.io/data/conduct_probs.csv){target="_blank"} 
...
Alongside looking at the correlation matrix, below are a couple of additional ways to do this.  
...
Alternatively, if you want a ggplot based approach:
```{r message=FALSE}
library(GGally)
...
Would you expect factors to correlate? If so, you'll need an oblique rotation.  
...
conduct_efa$Phi
```
We can see here that there is a moderate correlation between the two factors. An oblique rotation is appropriate here. If it was a weak correlation, then it wouldn't be differing much from if we used an orthogonal rotation.  
`r solend()`

`r qbegin(qcounter())`
Compare your three differen solutions: 
