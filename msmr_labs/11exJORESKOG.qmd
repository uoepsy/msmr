...
`r qbegin(qcounter())`
Read in the data and explore it. Look at the individual distributions to get a sense of univariate normality, as well as the number of response options each item has.  
...
Specifying any ordered-categorical variables in when fitting these models will switch it to using an appropriate estimation method.  

```{r}
#| eval: false
cfa(model.formula, data = mydata, ordered = c("item1","item2","item3"))
```
...
```
The joint probability distribution of these two variables can be visualised in @fig-mvnn. 
```{r}
#| echo: false
#| label: fig-mvnn
...
`r qbegin("A5")`
Fit a one factor CFA for the IQ items.  
  
Be sure to first check their distributions (both numerically and visually) and fit the CFA using an appropriate estimation method.   


::: {.callout-tip collapse="true"}
#### Hints

the `describe()` function from the __psych__ package will give you measures of skew and kurtosis.  
