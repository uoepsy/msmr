...
:::rtip 
__A Note on Convergence warnings__  
...
A frequent cause of confusion when learning about multilevel models is the use of the term "random effect". Does it refer to the the grouping variable, or to the effects that we allow to vary by-groups? The answer is really both. In the model below, the "random effects" refers to the `(1 + hrs_week | toy_type)`. We are specifying by-toy type random intercepts and by-toy type effects of hrs_week.
```{r}
library(tidyverse)
library(lme4)
...
# Assumptions & Influence in MLM

:::statbox
__Assumptions in Multilevel Models__   
...
`r optbegin("Looking at Level 1 Residuals", olabel=FALSE, toggle=params$TOGGLE)`
...
`r optend()`
`r optbegin("Looking at Level 2+ Residuals", olabel=FALSE, toggle=params$TOGGLE)`

TODO 
we need to index appropriately
...
:::statbox
__Influence in Multilevel Models__  
...
:::rtip
The most useful package for assessing influence in multilevel models is __HLMdiag__.  
:::

:::

`r optbegin("Examining level 1 influence",olabel=FALSE,toggle=params$TOGGLE)`
...
`r optend()`
`r optbegin("Examining level 2+ influence", olabel=FALSE, toggle=params$TOGGLE)`
