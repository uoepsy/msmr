...
We didn't specify `(Phase | ID)` as the random effect because each participant only has 1 data point for both Immediate and Follow-up. For example, for patient15, we have:

```{r}
subset(nwl_test, ID == 'patient15')
```

You couldn't compute an estimate of the standard deviation based on so few data points, remember the denominator of the standard deviation is $n - 1$! If you try using `(Phase | ID)` as random effect, you will see the following message:
