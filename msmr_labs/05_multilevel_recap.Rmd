...
# Urban effects on Aggression in Adolescence  
...
:::
`r qbegin()`
> How does aggressive behaviour change over adolescence? How is this change dependent upon the number of home_loc a child has?  

:::frame
__Data: Aggressive Behaviour in Adolescence__

Data was collected from 30 secondary schools across Scotland. A cohort of students were followed up every year from the age of 12 to 19. Each year, they completed the Aggressive Behaviour Scale (ABS). Data was also captured on the number of home_loc each child had.  
...
tibble(
  ...
  description = c("School Identifier","Score on the Aggressive Behaviour Scale (Z-scored)","Age (in years) of child at observation","Within-School Child Identifier","Number of home_loc (0, 1, 2, >2)")
) %>% knitr::kable()
```
:::
...
```{r eval=F}
ggplot(absdat, aes(x = year, y = ABS,col=home_loc)) +
  stat_summary(geom="pointrange",aes(group=home_loc),alpha=.5)+
  stat_summary(geom="line",aes(group=home_loc),alpha=.5)+
  labs(title="Average ABS scores over age by number of home_loc", x = "Age", y = "Aggressive Behaviour Score")
...
ggplot(absdat, aes(x = year, y = ABS,col=home_loc)) +
  ...
  facet_wrap(~home_loc, labeller = label_both) +
...
- We can group our data by number of home_loc (0,1,2,3+), by school, and by child.  
- We're specifically interested in differences between the levels of the `home_loc` variable, so that is going to be in our fixed effects. 
...
- Along with being interested in number of home_loc, we're also interested in change over time. From our initial plotting, we see there is some curvature present, and this is most apparent in the 3+ home_loc group. Are we interested in differences between sibling groups at the first timepoint (age 12?). Possibly? This depends a lot on the research field.   
- We know that effects of time can vary by child, and so also by school. Children don't change in how many home_loc they have (well, they might, not often), so we can't really think of "the effect on aggressive behaviours for child $i$, of having a different number of home_loc" because child $i$ always has the same number of home_loc.  However, within a school, different children have different numbers of home_loc, so it might be that this effect can vary by-school. 
