...
- In blue you see the overall line with the overall intercept $\gamma_{00}$ and slope $\gamma_{10}$

In green you see the line of a specific group (the green points). The green group has a line with a different intercept and slope from the overall line (in blue). Multilevel models specify group-specific lines in relation to the overall line. As such, the intercept of the group-specific line $\beta_{0i}$ is written as the overall intercept $\gamma_{00}$ plus a group-specific random adjustment $\zeta_{0i}$. Similarly, the group-specific slope $\beta_{1i}$ is written as the overall slope $\gamma_{10}$ plus a group-specific random adjustment $\zeta_{1i}$.

- The residuals $\epsilon_{ij}$ represent the deviations of each point to the group-specific line.

- The random adjustment to the intercept, $\zeta_{0i}$ is a random variable called the __random intercept__, and has mean zero and variance $\sigma^2_{\zeta_{0}} = \text{var}(\zeta_{0,\text{group1}}, ..., \zeta_{0,\text{groupG}})$.

- The random adjustment to the slope, $\zeta_{1i}$ is a random variable called the __random slope__, and has mean zero and variance $\sigma^2_{\zeta_{1}} = \text{var}(\zeta_{1,\text{group1}}, ..., \zeta_{1,\text{groupG}})$.
