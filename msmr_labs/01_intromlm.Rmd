...
```{r echo=FALSE, fig.cap = "[USMR Week 8 Lecture](https://uoepsy.github.io/usmr/2223/lectures/lecture_8.html#32)", out.width="300px",fig.align="center"}
...
& \qquad y_i = \color{blue}{\beta_0} \cdot 1 + \color{blue}{\beta_1} \cdot x_{1i} + \dots  + \color{blue}{\beta_p} \cdot x_{pi} + \color{red}{\varepsilon_i} \\ 
\end{align}
$$

We used the following colour-coding:

- Blue denotes a fixed population parameter
- Red denotes a random variable representing a deviation. In this case $\color{red}{\varepsilon_i}$ represents the error between a point and the line prediction.
...
$$
\mathbf{y} = \boldsymbol{X} \boldsymbol{\beta} + \boldsymbol{\varepsilon}
\quad
...
ggplot(data = toys_read, aes(x = hrs_week, y = R_AGE))+
  geom_point(alpha=.5)+
  geom_smooth(method = "lm")+
  geom_point(data=toys_read %>% filter(toy_type=="Scooby Doo"), color="red")+
  geom_segment(data=broom::augment(simplemod)[toys_read$toy_type=="Scooby Doo",], aes(y=R_AGE,xend=hrs_week,yend=.fitted,xend=hrs_week),lty="dashed",color="red")+
...
```{r}
ggplot(data = toys_read, aes(x=hrs_week, y=R_AGE, color=toy_type))+
  geom_point()+
  geom_smooth(method="lm",se=FALSE)
```
...
ggplot(toys_read, aes(x=hrs_week, y=R_AGE))+
  ...
  geom_point(data = filter(toys_read, str_detect(toy_type, "Scooby|Farm")), size=3, aes(color=toy_type))
```
...
library(broom)
augment(mod1) %>%
  ggplot(.,aes(x=hrs_week, y=.fitted, color=toy_type))+
  geom_line()
```
`r solend()`
...
\quad \\
& \text{level 1:} \\
& \qquad y_{ij} = \color{green}{\beta_{0i}} \cdot 1 + \color{green}{\beta_{1i}} \cdot x_{ij} + \color{red}{\varepsilon_{ij}} \\\\
& \text{level 2:} \\
& \qquad \color{green}{\beta_{0i}} = \color{blue}{\gamma_{00}} + \color{red}{\zeta_{0i}} \\
& \qquad \color{green}{\beta_{1i}} = \color{blue}{\gamma_{10}} + \color{red}{\zeta_{1i}} \\
\quad \\
& \text{where:} \\
& \qquad \color{blue}{\gamma_{00}} \text{ is the population intercept, and }\color{red}{\zeta_{0i}}\text{ is the deviation of group }i\text{ from }\color{blue}{\gamma_{00}} \\
& \qquad \color{blue}{\gamma_{10}} \text{ is the population slope, and }\color{red}{\zeta_{1i}}\text{ is the deviation of group }i\text{ from }\color{blue}{\gamma_{10}} \\
& \qquad \color{red}{\varepsilon_{ij}} \text{ is the deviation between a point and the line of the group that point belongs to}
\end{align}
$$

We are now assuming $\color{red}{\zeta_0}$, $\color{red}{\zeta_1}$, and $\color{red}{\varepsilon}$ to be normally distributed with a mean of 0, and we denote their variances as $\sigma_{\zeta_0}^2$, $\sigma_{\zeta_1}^2$, $\sigma_{\varepsilon}^2$ respectively. 

The $\color{red}{\zeta}$ components also get termed the "random effects" part of the model, Hence names like "random effects model", etc. 

We used the following colour-coding:

- Blue denotes a fixed population parameter (overall parameter)
- Red denotes a random variable representing a deviation / adjustment
- Green denotes a random variable representing a group-specific parameter

__Visually__

```{r echo=FALSE, out.width = "100%"}
knitr::include_graphics("images/lmm_visual.png")
```

- In blue you see the overall line with the overall intercept $\color{blue}{\gamma_{00}}$ and slope $\color{blue}{\gamma_{10}}$

- In green you see the line of a specific group (the green points). The green group has a line with a different intercept and slope from the overall line (in blue). Multilevel models specify group-specific lines in relation to the overall line. As such, the intercept of the group-specific line $\color{green}{\beta_{0i}}$ is written as the overall intercept $\color{blue}{\gamma_{00}}$ plus a group-specific random adjustment $\color{red}{\zeta_{0i}}$. Similarly, the group-specific slope $\color{green}{\beta_{1i}}$ is written as the overall slope $\color{blue}{\gamma_{10}}$ plus a group-specific random adjustment $\color{red}{\zeta_{1i}}$.

- The residuals $\color{red}{\varepsilon_{ij}}$ represent the deviations of each point to the group-specific line.

- The random adjustment to the intercept, $\color{red}{\zeta_{0i}}$ is a random variable called the __random intercept__, and has mean zero and variance $\sigma^2_{\zeta_{0}} = \text{Var}(\zeta_{0,\text{group1}}, ..., \zeta_{0,\text{groupG}})$.

- The random adjustment to the slope, $\color{red}{\zeta_{1i}}$ is a random variable called the __random slope__, and has mean zero and variance $\sigma^2_{\zeta_{1}} = \text{Var}(\zeta_{1,\text{group1}}, ..., \zeta_{1,\text{groupG}})$.
...
$$
y_{ij} = 
    (\color{blue}{\beta_0} + \color{red}{\zeta_{0i}}) \cdot 1 + 
    ( \color{blue}{\beta_{1}} + \color{red}{\zeta_{1i}} ) \cdot x_{ij} +
    \color{red}{\varepsilon_{ij}} \\
$$

- In various resources, you are likely to see $\color{blue}{\alpha}$ used to denote the intercept instead of $\color{blue}{\beta_0}$. 

- Many people use the symbol $\color{red}{u}$ in place of $\color{red}{\zeta}$.

- Some people use the condensed matrix form of the model, in which the $\boldsymbol{Z}$ matrix represents the grouping structure, and the $\color{red}{\boldsymbol{u}}$ (or $\color{red}{\boldsymbol{\zeta}}$) are the estimated random deviations. 

$$
\mathbf{y} = 
    \boldsymbol{X}\color{blue}{\boldsymbol{\beta}} + 
    \boldsymbol{Z}\color{red}{\boldsymbol{u}} + 
...
With **lme4**, we now have the addition of __random effect terms__, specified in parenthesis with the `|` operator (the vertical line | is often found to the left of the z key on QWERTY keyboards).  
...
\begin{align}
& \text{level 1:} \\
& \qquad y_{ij} = \color{green}{\beta_{0i}} \cdot 1 + \color{blue}{\beta_{1}} \cdot x_{ij} + \color{red}{\varepsilon_{ij}} \\
& \text{level 2:} \\
& \qquad \color{green}{\beta_{0i}} = \color{blue}{\gamma_{00}} + \color{red}{\zeta_{0i}} \\
\end{align}
$$
...
& \text{level 1:} \\
    & \qquad y_{ij} = 
        \color{green}{\beta_{0i}} \cdot 1 + \color{green}{\beta_{1i}} \cdot x_{ij} + \color{red}{\varepsilon_{ij}} \\
& \text{level 2:} \\
        & \qquad \color{green}{\beta_{0i}} = 
            \color{blue}{\gamma_{00}} + \color{red}{\zeta_{0i}} \\
        & \qquad \color{green}{\beta_{1i}} = 
            \color{blue}{\gamma_{10}} + \color{red}{\zeta_{1i}} \\
\end{align}
$$
:::
:::
...
Remember back to [Week 10 of USMR](https://uoepsy.github.io/usmr/2223/labs/10a_glm.html), when we introduced the generalised linear model (GLM) we briefly discussed **Maximum likelihood** in an explanation of how models are fitted.  
...
$$
\text{ReadingAge}_i = \color{blue}{\beta_0} \cdot 1 + \color{blue}{\beta_1} \cdot\text{Practice}_{i} + \color{red}{\varepsilon_i}
$$
...
\\
&\text{level 1:}\\
    & \qquad \text{ReadingAge}_{ij} = \color{green}{\beta_{0i}} \cdot 1 + \color{blue}{\beta_{1}} \cdot \text{Practice}_{ij} + \color{red}{\varepsilon_{ij}} \\
\\
&\text{level 2:}\\
    & \qquad \color{green}{\beta_{0i}} = \color{blue}{\gamma_{00}} + \color{red}{\zeta_{0i}} \\
\end{align}
...
\quad \\
& \text{where:} \\
    & \qquad \color{red}{\zeta_0} \sim N(0, \sigma_{\zeta_{0}})  \text{ independently} \\
    & \qquad \color{red}{\varepsilon} \sim N(0, \sigma_{\varepsilon}) \text{ independently} \\
\end{align}
$$
...
The circles in Figure \@ref(fig:lmersummap) correspond to:

+ Yellow = $\sigma_{\zeta_{0}}$   
...
- Information on the conditions for each trial for each participant is stored in __.csv__ format at https://uoepsy.github.io/data/basketballconditions_msmr.csv (data collected is detailed in the table below).  
- Information on participants' HRV for each trial is stored in **.csv** format, and can be downloaded from https://uoepsy.github.io/data/bballhrv_msmr.csv 


```{r echo=F}
data.frame(
  variable = names(read_csv("https://uoepsy.github.io/data/basketballconditions_msmr.csv")),
...
bball <- 
  left_join(
    read_csv("https://uoepsy.github.io/data/basketballconditions_msmr.csv"),
    read_csv("https://uoepsy.github.io/data/bballhrv_msmr.csv") %>%
...
**Tip:** If you get an error about model convergence, consider changing the optimiser (see [above](01_intromlm.html#Estimation))
`r qend()`
`r solbegin(show=params$SHOW_SOLS, toggle=params$TOGGLE)`
