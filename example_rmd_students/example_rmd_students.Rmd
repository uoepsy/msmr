...
<!-- This is a text comment in Rmd. It's not the same as code comments, which -->
<!-- need to be inside a code chunk and use # my comment after the hashtag -->
<!-- [Display text](link) is how we do hyperlink in Rmd -->
...
knitr::include_graphics("https://uoepsy.github.io/msmr/labs/images/intro/reading.png")
```
...
knitr::include_graphics("https://uoepsy.github.io/msmr/labs/images/intro/toys.png")
```
...
tibble(
    variable=names(toys_read),
    description=c("Type of Toy",
   "Character",
...
$$
\text{for observation }i,  \qquad
Y_i = \beta_0 \cdot 1 + \beta_1 X_{1i} + \cdots + \beta_p \cdot X_{pi} + \epsilon_i
$$
...
$$
\begin{align}
& \text{for observation }i\text{ in group }j \\
\quad \\
& \text{Level 1:} \\
& Y_{ij} = \beta_{0j} \cdot 1 + \beta_{1j} \cdot X_{ij} + \epsilon_{ij} \\
& \text{Level 2:} \\
& \beta_{0j} = \gamma_{00} + \zeta_{0j} \\
& \beta_{1j} = \gamma_{10} + \zeta_{1j} \\
\quad \\
& \text{Where:} \\
& \gamma_{00}\text{ is the population intercept, and }\zeta_{0j}\text{ is the deviation of group }j\text{ from }\gamma_{00} \\
& \gamma_{10}\text{ is the population slope, and }\zeta_{1j}\text{ is the deviation of group }j\text{ from }\gamma_{10} \\
\end{align}
$$
...
$$
Y_{ij} 
= \underbrace{(\beta_0 + \zeta_{0j})}_{\text{intercept of group j}} \cdot 1 
+ \underbrace{( \beta_{1} + \zeta_{1j} )}_{\text{slope of group j}} \cdot X_{ij}  +  \varepsilon_{ij}
$$
...
\begin{align}
& \text{Level 1:} \\
& Y_{ij} = \beta_{0j} \cdot 1 + \beta_{1} \cdot X_{ij} + \epsilon_{ij} \\
& \text{Level 2:} \\
& \beta_{0j} = \gamma_{00} + \zeta_{0j} \\
\end{align}
$$
...
$$
ReadingAge_i = \beta_0 \cdot 1 + \beta_1 \cdot Practice_{1i} + \epsilon_i
$$
...
knitr::include_graphics("https://uoepsy.github.io/msmr/labs/images/intro/mle.png")
```
...
knitr::include_graphics("https://uoepsy.github.io/msmr/labs/images/intro/mle2.png")
```
...
knitr::include_graphics("https://uoepsy.github.io/msmr/labs/images/intro/summarylmer.png")
```
...
\begin{align}
& \text{Level 1:} \\
& ReadingAge_{ij} = \beta_{0j} \cdot 1 + \beta_{1} \cdot Practice_{ij} + \epsilon_{ij} \\
& \text{Level 2:} \\
& \beta_{0j} = \gamma_{00} + \zeta_{0j} \\
\quad \\
& \text{where} \\
& \zeta_0 \sim N(0, \sigma_{\zeta_{0}})  \text{ independently} \\
